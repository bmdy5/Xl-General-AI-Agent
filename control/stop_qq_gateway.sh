#!/bin/bash
# MyAgent - 手动停止 QQ Gateway 脚本
# 职责: 安全、彻底地杀掉 QQ 网关服务。支持 launchd 系统级卸载与后台进程强杀。
...
# 3. 验证状态
if ps aux | grep -q "[m]ain.py --gateway"; then
    echo "❌ QQ 网关进程终止失败，请手动检查！"
    exit 1
else
    echo "✅ QQ Gateway 停止成功！"
fi
echo "=========================================="
