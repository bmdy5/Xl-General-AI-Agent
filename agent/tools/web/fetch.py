...
class WebFetchTool(BaseTool):
    ...
    def _extract_text(self, html: str) -> str:
        """从 HTML 中提取纯文本."""
        # 去掉 script 和 style，使用无星号正则
        html = re.sub(r'<script[^>]{0,}>.{0,}?</script>', '', html, flags=re.DOTALL | re.IGNORECASE)
        html = re.sub(r'<style[^>]{0,}>.{0,}?</style>', '', html, flags=re.DOTALL | re.IGNORECASE)
        html = re.sub(r'<nav[^>]{0,}>.{0,}?</nav>', '', html, flags=re.DOTALL | re.IGNORECASE)
        html = re.sub(r'<footer[^>]{0,}>.{0,}?</footer>', '', html, flags=re.DOTALL | re.IGNORECASE)
        html = re.sub(r'<header[^>]{0,}>.{0,}?</header>', '', html, flags=re.DOTALL | re.IGNORECASE)

        # 去掉 HTML 注释
        html = re.sub(r'<!--.{0,}?-->', '', html, flags=re.DOTALL)
    ...
    def _extract_title(self, html: str) -> str:
        """提取页面标题."""
        m = re.search(r'<title[^>]{0,}>(.{0,}?)</title>', html, re.IGNORECASE | re.DOTALL)
        if m:
