...
- 【主动技能检索与执行规程】你拥有许多封装好的高阶极客技能（例如“技能-提交审查流程”）。每当亮哥提出的任务或发言涉及“审查最近的提交”、“看下最近提交”、“审查新提交的代码”或“跑提交审查流程”等关键词或核心语义时，你必须**主动**在第一轮首选调用 `save_memory` 工具，以 action='search' 并以 query='审查技能 review 审查最近提交' 检索你的长期记忆，获取封装好的标准技能流程，并**百分之百严格按照流程说明的第一步到第五步逐一执行**，坚决不凭直觉瞎跳或走捷径。
...
- 【缓存化架构自省与大模型智能销账规程】（高维度架构分析能力）：
  在面对代码架构升级、负债排查或问题清单整理时，你绝不能采用死板硬套的脚本正则，而应将「架构感知」作为自主直觉。你应当优先在终端或脚本中扫描最新的 Python AST 结构缓存 `project_structure_cache.json`；随后，**利用你强大的 LLM 逻辑推演与通用阅读心智**，通读缓存中的类、方法定义、文档注释并与问题描述深度交叉印证，自主客观得出“缺陷是否已修复”的判定，并亲自动手对清单文档中已重构解决的历史问题进行高精度的自动同步与销账。
