...
class StreamPresenter:
    ...
    async def handle_delta(self, content: str, msg_type: str, user_id: str, group_id: str):
        ...
        if self.is_voice_reply:
            ...
            if "[SPLIT]" in self.buf:
                ...
                for part in parts[:-1]:
                    if part.strip():
                        self.dispatcher._log_activity_dispatcher("AI 计划/答复", part.strip(), user_id=user_id)
                        await self.context.send_chunk(msg_type, user_id, group_id, part.strip())
                        self.total_sent_tokens += self.dispatcher._count_tokens(part.strip())
                self.buf = parts[-1]
            elif "\n\n" in self.buf and len(self.buf) > 40:
                idx = self.buf.rfind("\n\n")
                to_send = self.buf[:idx]
                if to_send.strip():
                    self.dispatcher._log_activity_dispatcher("AI 计划/答复", to_send.strip(), user_id=user_id)
                    await self.context.send_chunk(msg_type, user_id, group_id, to_send.strip())
                    self.total_sent_tokens += self.dispatcher._count_tokens(to_send.strip())
    ...
    async def flush_buffer(self, msg_type: str, user_id: str, group_id: str):
        ...
        if self.buf.strip():
            self.dispatcher._log_activity_dispatcher("AI 计划/答复", self.buf.strip(), user_id=user_id)
            if self.is_voice_reply:
                ...
                if pure_text:
                    ...
                    self.total_sent_tokens += self.dispatcher._count_tokens(pure_text)
            else:
                await self.context.send_chunk(msg_type, user_id, group_id, self.buf.strip())
                self.total_sent_tokens += self.dispatcher._count_tokens(self.buf.strip())
            self.buf = ""
