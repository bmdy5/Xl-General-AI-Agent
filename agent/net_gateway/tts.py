...
logger = logging.getLogger("net_gateway.tts")
...
async def send_voice(context, msg_type: str, user_id: str, group_id: str, text: str, style: str = "知性", is_test: bool = False):
    """通过 GPT-SoVITS 专属音轨服务异步合成语音，再通过 context 中转给 QQ"""
    if not text.strip():
        return
    ...
    try:
        # 2. 文本清洗，过滤特殊的 Markdown 标记，以及旁白动作括号“（动作描述）”
        clean = voice_text
        clean = re.sub(r'（[^）]+）', '', clean)  # 过滤中文括号及其内容
        clean = re.sub(r'\([^)]+\)', '', clean)  # 过滤英文括号及其内容
        clean = re.sub(r'\[[^\]]+\]', '', clean)  # 过滤情感标签，确保不念出杂音
