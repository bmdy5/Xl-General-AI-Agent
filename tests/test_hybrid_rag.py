...
async def test_hybrid_rag_flow():
    ...
    # 1. 备份原始热迁移类方法，并在测试后安全复原，防范全局测试进程数据污染
    original_migration = MemoryManager._run_hot_migration_if_needed
    MemoryManager._run_hot_migration_if_needed = lambda *args, **kwargs: None
    
    # 2. 物理重定向 settings 中的备份路径，彻底切断“换家自愈还原引擎”从真实备份库获取污染数据的路径
    from agent.core.config import settings
    original_memory_cfg = settings._data.get("memory", {}).copy()
    
    project_root = Path(__file__).resolve().parent.parent
    temp_mem_dir = project_root / "temp_test_hybrid_rag"
    temp_bak_dir = project_root / "temp_test_hybrid_rag_bak"
    
    if temp_mem_dir.exists():
        shutil.rmtree(temp_mem_dir)
    temp_mem_dir.mkdir(parents=True, exist_ok=True)
    
    if temp_bak_dir.exists():
        shutil.rmtree(temp_bak_dir)
    temp_bak_dir.mkdir(parents=True, exist_ok=True)
    
    settings._data["memory"] = {
        "base_dir": str(temp_mem_dir),
        "backup_dir": str(temp_bak_dir),
        "auto_backup_to_project": False,
    ...
    try:
        ...
        assert duration < 0.30, "Hybrid search should complete within 300ms (prevent occasional CPU overload fails)"
        ...
        # 3. 黄金收尾：彻底清除临时目录，复原全局设置与类属性，保持环境极致纯净
        if temp_mem_dir.exists():
            shutil.rmtree(temp_mem_dir)
        if temp_bak_dir.exists():
            shutil.rmtree(temp_bak_dir)
            
        settings._data["memory"] = original_memory_cfg
        MemoryManager._run_hot_migration_if_needed = original_migration
