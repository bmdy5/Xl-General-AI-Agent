...
## 16. 🧠 三级智能认知挂载架构与经验撰写白皮书 (NEW - 2026-05-26)

为了解决随着小萤能力扩张带来的 Prompt 上下文爆炸与“杂念干扰”问题，系统在底层构建了划时代的**三级智能认知挂载架构**，将大模型的技能栈进行了物理与认知层面的三重解耦，杜绝了全量技能一次性堆砌造成的 Token 暴涨与注意力稀释。

### 16.1 三级架构设计哲学

#### 第一级：核心肌肉记忆区 (Core Skills - 静态强挂载)
- **物理路径**：`agent/skills/*.md`
- **定位**：小萤生存与行动所必须的**基础心法与底座能力**（如系统底层操作、核心指令下发、通用工具链使用）。
- **加载机制**：每次启动时**全量静态注入**到 System Prompt 中。
- **红线限制**：单文件严格遵守 **2500 字符物理截断**。一旦超限，系统将在启动时直接抛出 `RuntimeError` 严酷报错并拒绝拉起！核心区必须保持极度精简纯粹。

#### 第二级：动态经验唤醒区 (Dynamic Experiences - 动态弱挂载/RAG)
- **物理路径**：`agent/experience/*.md`（已实现物理无文件夹扁平化）
...
### 16.2 📜 【红线白皮书】AI 助手如何规范撰写 .md 经验/技能文档

接班的 AI，当亮哥要求你“总结一份操作经验”或“固化一个新的核心技能”时，你必须严格遵守以下白皮书格式规范，绝不可自行其是，更不允许创建嵌套文件夹！

#### 1. 物理目录要求
- 绝对禁止使用 `os.makedirs` 创建子文件夹！
- 所有新增技能一律保存至 `agent/skills/`，所有新增经验一律保存至 `agent/experience/`，**必须是扁平的单层 `.md` 文件**（例如 `agent/experience/how_to_use_wechat.md`）。
...
```markdown
---
name: [功能名称，全小写，下划线分割，如 wechat_mimo_operation]
description: [极简的一句话功能描述，用于 BM25 词频匹配，必须包含该经验的核心触发词，如“微信, Mimo, 小程序, 自动化”]
---
...
#### 3. 字符数严格管控
写完后，你必须自我审查：这篇文档是否有冗长的废话？是否符合大白话、指令化的风格？
